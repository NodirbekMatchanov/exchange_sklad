...
use rare\exchange1c\behaviors\BomBehavior;
use rare\exchange1c\ExchangeEvent;
use rare\exchange1c\ExchangeModule;
use rare\exchange1c\helpers\ByteHelper;
use rare\exchange1c\helpers\NodeHelper;
use rare\exchange1c\helpers\SerializeHelper;
use rare\exchange1c\interfaces\DocumentInterface;
use rare\exchange1c\interfaces\OfferInterface;
use rare\exchange1c\interfaces\ProductInterface;
use Yii;
...
class ApiController extends Controller
{
    ...
    public function actionFile($type, $filename)
    {
        ...
        $filePath = $this->module->getTmpDir() . DIRECTORY_SEPARATOR . Yii::$app->user->identity->getId() .DIRECTORY_SEPARATOR . $filename;
    ...
    public function parsingImport($file)
    {
        ...
        $classifierFile = Yii::getAlias($this->module->tmpDir .DIRECTORY_SEPARATOR . Yii::$app->user->identity->getId() . '/classifier.xml');
     ...
     * @return bool
     */
    public function actionImport($type, $filename, $auth)
    {
        $file = $this->module->getTmpDir() .DIRECTORY_SEPARATOR . Yii::$app->user->identity->getId() . DIRECTORY_SEPARATOR . $filename;
        switch ($type) {
            case 'catalog':
    ...
    protected function parseImage($model, $product)
    {
        ...
        foreach ($images as $image) {
            $path = realpath($this->module->getTmpDir() .DIRECTORY_SEPARATOR . Yii::$app->user->identity->getId() . DIRECTORY_SEPARATOR . $image->path);
            if (file_exists($path)) {
    ...
    /**
     * @return \rare\exchange1c\interfaces\GroupInterface
     */
